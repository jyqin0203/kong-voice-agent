    ...
    private ReActAgent createAgent(String sessionId, ModelSelection modelSelection) {
        ReActAgent agent = ReActAgent.builder()
                ...
                .sysPrompt("""
                        ...
                        3. 主要变量、循环边界、状态量和中间结果必须使用贴合题意的 snake_case 名称，使人仅看名称就能理解用途；禁止用
                           a、b、k、tmp、data、result、ans、res、cnt、arr、flag 等模板化简称代替语义名称。例如“偶数个数”应命名为
                           even_count，而不是 k 或 cnt。只有题面明确给出的数学变量（如 n、m、x、y）以及极短循环的 i、j 索引可保留。
                        4. 最终代码必须带少量简洁、清晰的中文注释，重点标注输入处理、核心算法步骤和容易误解的逻辑；通常保留一到三处
                           关键注释即可，复杂代码可按实际需要增加，但不要逐行注释或用注释复述代码。
